...
function Card({
  ...
  return (
    <Link
      to="/article/$slug"
      params={{ slug: s.slug }}
      className="bg-white rounded-xl ring-1 ring-navy/5 shadow-editorial overflow-hidden flex hover:bg-navy/5 transition group"
    >
      <img
        src={s.image}
        alt=""
      ...
      <div className="p-3 flex-1 min-w-0 relative">
        <span
          className={`absolute top-3 right-3 size-6 rounded-full grid place-items-center ring-2 bg-paper ${color}`}
        >
          {isDone ? (
        ...
        <div
          className={`inline-block text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${color}`}
        >
          {s.category?.name || "Impact"}
        </div>
        <p className="font-hindi text-navy text-sm leading-snug mt-1.5 pr-8 group-hover:text-orange transition-colors line-clamp-2">
          {s.title}
        </p>
      </div>
    </Link>
...
export function ImpactTimeline({ items }: { items: DynamicArticle[] }) {
  ...
  return (
    <section className="bg-paper py-12 px-4">
      <div className="mb-6">
        <div className="text-orange text-[10px] font-bold uppercase tracking-[0.3em] mb-1">
          Impact Series
        </div>
        <h2 className="font-hindi text-2xl font-semibold text-navy">
          समस्या से समाधान तक
        </h2>
        <p className="text-navy/50 text-xs font-body-hindi mt-2">
          एक कहानी, अनेक पड़ाव — बुंदेलखंड का असली बदलाव
        </p>
      </div>
      ...
      <Link
        ...
        className="mt-6 w-full flex items-center justify-center gap-2 bg-navy text-paper rounded-full py-3 text-xs font-bold uppercase tracking-widest hover:bg-navy/90 transition shadow-elevated"
      >
