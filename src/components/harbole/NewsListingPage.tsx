...
export function NewsListingPage({ jilaSlug, subDistrictSlug, categorySlug }: Props) {
  const { data: navData, isLoading: navLoading } = useNavigationData();
  const { data: articles, isLoading: articlesLoading } = useArticles(jilaSlug, subDistrictSlug, categorySlug);
  const navigate = useNavigate();

  const selectedJila = navData?.districts?.find(d => d.slug === jilaSlug);
  const selectedSubDistrict = navData?.sub_districts?.find(sd => sd.slug === subDistrictSlug);
  const selectedCategory = navData?.categories?.find(c => c.slug === categorySlug);
  ...
  const removeFilter = (type: 'jila' | 'subDistrict' | 'category') => {
    let url = "";
    if (type === 'jila') {
      url = categorySlug ? `/category/${encodeURIComponent(categorySlug)}` : '/';
    } else if (type === 'subDistrict') {
      url = `/news/${encodeURIComponent(jilaSlug!)}`;
      if (categorySlug) url += `/category/${encodeURIComponent(categorySlug)}`;
    } else if (type === 'category') {
      if (jilaSlug && subDistrictSlug) url = `/news/${encodeURIComponent(jilaSlug)}/district/${encodeURIComponent(subDistrictSlug)}`;
  ...
  const getCategoryUrl = (cSlug: string) => {
    if (jilaSlug && subDistrictSlug) return `/news/${encodeURIComponent(jilaSlug)}/district/${encodeURIComponent(subDistrictSlug)}/category/${encodeURIComponent(cSlug)}`;
    if (jilaSlug) return `/news/${encodeURIComponent(jilaSlug)}/category/${encodeURIComponent(cSlug)}`;
  ...
  return (
    <div className="min-h-screen bg-paper">
      <div className="max-w-[480px] md:max-w-5xl lg:max-w-6xl xl:max-w-7xl mx-auto bg-paper relative">
        <Header activeJilaSlug={jilaSlug} activeCategorySlug={categorySlug} activeSubDistrictSlug={subDistrictSlug} />
        <main className="flex flex-col md:flex-row pb-32 md:pb-16 min-h-[70vh]">
          ...
          <aside className="w-full md:w-64 lg:w-72 shrink-0 p-4 md:p-6 border-r border-navy/5">
            ...
            {(selectedJila || selectedCategory) && (
              <div className="mb-8">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-navy/40 mb-3">Active Filters</h3>
                <div className="flex flex-wrap gap-2">
                  {selectedJila && (
                    <button onClick={() => removeFilter('jila')} className="flex items-center gap-1.5 px-2.5 py-1 bg-navy text-paper text-xs font-semibold rounded-full hover:opacity-90">
                      {selectedJila.name} <X className="size-3" />
                    </button>
                  )}
                  {selectedSubDistrict && (
                    <button onClick={() => removeFilter('subDistrict')} className="flex items-center gap-1.5 px-2.5 py-1 bg-navy text-paper text-xs font-semibold rounded-full hover:opacity-90">
                      {selectedSubDistrict.name} <X className="size-3" />
                    </button>
                  )}
                  {selectedCategory && (
                    <button onClick={() => removeFilter('category')} className="flex items-center gap-1.5 px-2.5 py-1 bg-orange text-paper text-xs font-semibold rounded-full hover:opacity-90">
            ...
            {selectedJila && displaySubDistricts.length > 0 && (
              <div className="mb-8">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-navy/40 mb-3">Districts</h3>
            ...
            {displayCategories.length > 0 && (
              <div>
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-navy/40 mb-3">Categories</h3>
                <div className="flex flex-col gap-1.5">
          ...
          <div className="flex-1 p-4 md:p-6 lg:p-8">
            <header className="mb-8 pb-6 border-b border-navy/10">
              <h1 className="font-hindi text-3xl md:text-5xl font-medium text-navy leading-none">
                {selectedCategory ? selectedCategory.name : (selectedSubDistrict?.name || selectedJila?.name || "News")}
              </h1>
              {selectedJila && selectedCategory && (
                <p className="font-body-hindi text-navy/55 text-sm md:text-base mt-3">
                  {selectedSubDistrict ? `${selectedSubDistrict.name}, ${selectedJila.name}` : selectedJila.name} की ताज़ा ख़बरें
                </p>
              )}
            </header>
            ...
            {articlesLoading ? (
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                 {[...Array(6)].map((_, i) => (
                   <div key={i} className="animate-pulse bg-navy/5 aspect-[4/3] rounded-2xl"></div>
                 ))}
               </div>
            ) : articles?.length === 0 ? (
