...
export function AmitTripathiShow({
  ...
  const [activeEpisode, setActiveEpisode] = useState<DynamicEpisode | null>(
    featuredEpisode || pastEpisodes[0] || null,
  );
      ...
      <div className="relative bg-[oklch(0.16_0.035_250)] text-paper px-4 py-12 grain">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-orange text-[10px] font-bold uppercase tracking-[0.35em]">
              Flagship Show
            </span>
            <span className="size-1 rounded-full bg-paper/30" />
            {activeEpisode && (
              <span className="text-paper/50 text-[10px] uppercase tracking-widest font-semibold">
          ...
          <p className="font-body-hindi text-paper/65 mt-3 max-w-prose text-sm">
            हर सप्ताह बुंदेलखंड की धड़कनों से जुड़ी एक नई कहानी, एक तीखी बहस और
            एक ज़रूरी सवाल।
          </p>
                ...
                {isPlaying ? (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 grid place-items-center">
                      <div className="relative">
                        ...
                        <div className="relative size-20 rounded-full bg-orange grid place-items-center shadow-elevated ring-4 ring-orange/20">
                          <Play
                            className="size-8 text-paper fill-paper ml-1"
                            strokeWidth={1.5}
                          />
                        </div>
                      </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      <div className="text-gold text-[10px] font-bold uppercase tracking-[0.3em] mb-2">
                        Episode {activeEpisode.episode_number}
                      </div>
                      <h3 className="font-hindi text-xl text-paper leading-tight text-balance">
                        {activeEpisode.title}
                      </h3>
                      {activeEpisode.subtitle && (
                        <p className="font-hindi text-sm text-paper/80 mt-1">
                          {activeEpisode.subtitle}
                        </p>
                      )}
          ...
          {pastEpisodes.length > 0 && (
            <div className="mt-6">
              <div className="text-gold text-[10px] font-bold uppercase tracking-[0.3em] mb-3">
                Past Episodes
              </div>
              <div className="space-y-2.5">
                {pastEpisodes
                  .filter((e) => e.id !== activeEpisode?.id)
                  .map((e) => (
                    <button
                      key={e.id}
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[9px] font-bold uppercase tracking-widest text-gold mb-0.5">
                          EP {e.episode_number}
                        </div>
                        <p className="font-hindi text-sm text-paper leading-snug line-clamp-1">
                          {e.title}
                        </p>
                      </div>
                      {e.schedule && (
                        <span className="text-[10px] text-paper/40 font-semibold shrink-0">
                          {e.schedule}
                        </span>
                      )}
                    </button>
