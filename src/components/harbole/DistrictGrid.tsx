      ...
      <section className="px-4 py-10">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="aspect-square bg-navy/5 animate-pulse rounded-lg"
            />
          ))}
        </div>
      </section>
    ...
    <section className="px-4 py-10">
      <div className="mb-5">
        ...
        <p className="text-navy/50 text-xs font-body-hindi mt-2">
          अपने जिले की ख़ास ख़बरें सबसे ऊपर देखें
        </p>
      </div>
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2">
        {districts.map((d) => {
          ...
          return (
            ...
            >
              ...
              <MapPin
                className={`absolute top-2 left-2 size-3 z-10 ${active ? "text-gold" : "text-orange"}`}
              />
              <div className="relative z-10 w-full h-full flex items-center justify-center">
                <div
                  className={`font-hindi text-lg sm:text-xl font-semibold leading-tight ${active ? "text-paper" : "text-navy"}`}
                >
                  {d.name}
                </div>
              </div>
              {active && (
                <div className="absolute top-1.5 right-1.5 size-1 rounded-full bg-gold animate-pulse z-10" />
              )}
            </button>
          );
        })}
      ...
      <button className="mt-5 w-full flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-orange">
        देखें {activeDistrict?.name} की ख़बरें{" "}
