  ...
  return (
    <div
      className={`relative overflow-hidden border-y ${
        invert
          ? "bg-navy text-paper border-white/10"
          ...
          <div className="flex gap-10 whitespace-nowrap animate-marquee w-max hover:[animation-play-state:paused]">
            {loop.map((it, idx) => (
              <Link
                ...
                to="/article/$slug"
                params={{ slug: it.slug }}
                className="flex items-center gap-2 text-sm font-body-hindi group"
              >
                <span
                  className={`text-[9px] font-bold uppercase tracking-widest ${invert ? "text-gold" : "text-orange"}`}
                >
                  {it.district?.name || it.category?.name || "ताज़ा ख़बर"}
                </span>
                <span
                  className={`${invert ? "text-paper/90 group-hover:text-gold" : "text-navy/85 group-hover:text-orange"} transition-colors`}
                >
                  {it.title}
                </span>
                <span
                  className={`text-[10px] ${invert ? "text-paper/40" : "text-navy/35"}`}
                >
                  · {it.time}
                </span>
                <span
                  className={`size-1 rounded-full ${invert ? "bg-paper/30" : "bg-navy/20"}`}
                />
              </Link>
            ))}
          </div>
