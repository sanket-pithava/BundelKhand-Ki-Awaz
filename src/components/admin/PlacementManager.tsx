...
export function PlacementManager({ resource }: { resource: any }) {
  ...
  async function load() {
    ...
    const { data, error } = await (supabase as any)
      .from(resource.table)
      .select("id, sort_order, article_id, article:articles(title, image_url, category:categories(name), district:districts(name))")
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm text-navy/60">{loading ? "Loading…" : `${rows.length} article(s) in ${resource.label}`}</div>
        <button onClick={() => setPickerOpen(true)} className="flex items-center gap-2 rounded-lg bg-navy px-4 py-2 text-sm font-semibold text-paper hover:bg-navy/90 transition-colors">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r, i) => {
              ...
              const catName = Array.isArray(article?.category) ? article.category[0]?.name : article?.category?.name;
              ...
              return (
                <tr key={r.id} className="border-t border-navy/5 group">
                  ...
                  <td className="px-3 py-3">
                    <div className="font-semibold text-navy line-clamp-1">{article?.title || "Unknown Article"}</div>
                    <div className="text-[10px] uppercase tracking-wider text-navy/50 mt-0.5">{catName || "Uncategorized"}</div>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <button onClick={() => remove(r.id)} className="rounded px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-10 text-center text-navy/50">No articles assigned to {resource.label} yet. Click "Add Article" to assign one.</td></tr>
            )}
          </tbody>
        </table>
      </div>
