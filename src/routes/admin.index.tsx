...
const RESOURCES: Resource[] = [
  ...
  },
  {
    ...
    fields: [
      { key: "name", label: "Name", type: "text" },
      {
        ...
        label: "Slug (unique)",
        type: "text",
        placeholder: "e.g. atal-bihari-vajpayee",
      },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "image_url",
        label: "Thumbnail Override (Optional, auto-extracted for YT)",
        type: "text",
      },
      {
        key: "views",
        label: "View Count (Manual)",
        type: "text",
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "schedule",
        label: "Display Duration/Schedule",
        type: "text",
        placeholder: "48 मिनट",
      },
      {
  ...
  },
  {
    ...
    defaults: {
      is_done: false,
      sort_order: 0,
      color: "bg-orange/15 text-orange ring-orange/30",
    },
    fields: [
      {
        key: "tag",
        label: "Tag (समस्या/रिपोर्ट/कार्रवाई/समाधान)",
        type: "text",
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "sponsor", label: "Sponsor", type: "text" },
      {
        ...
        label: "Variant",
        type: "select",
        options: ["gold", "navy", "orange", "paper"],
      },
      {
        key: "placement",
        label: "Placement",
      ...
      {
        key: "image_url",
        label: "Desktop Banner Image (1920x800)",
        type: "image",
      },
      {
        key: "mobile_image_url",
        label: "Mobile Banner Image (1080x1350)",
        type: "image",
      },
...
import { ReporterDashboard } from "@/components/admin/ReporterDashboard";
import { UserManagement } from "@/components/admin/UserManagement";
import { PendingNewsManager } from "@/components/admin/PendingNewsManager";

function AdminHome() {
  const { isReporter } = useAdminAuth();
      ...
      <aside className="w-full lg:w-64 shrink-0 space-y-8">
        <header className="px-2">
          ...
          <p className="text-xs text-navy/60 mt-1">
            Manage every section of Harbole
          </p>
        </header>
      ...
      <main className="flex-1 min-w-0 bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-navy/5">
        <div className="mb-6 pb-6 border-b border-navy/5">
          ...
          <p className="text-sm text-navy/50 mt-1">
            Add, edit, or remove {resource.label.toLowerCase()} entries
            displayed on the platform.
          </p>
        </div>
        {resource.key === "articles" ? (
          <ArticleManager key={resource.key} resource={resource} />
        ) : resource.key === "user_management" ? (
          <UserManagement key={resource.key} />
        ) : resource.key === "pending_news" ? (
          <PendingNewsManager key={resource.key} />
        ) : resource.key === "hero" ||
          resource.key === "breaking" ||
          resource.key === "top10" ? (
                ...
                <tr key={r.id} className="border-t border-navy/5">
                  <td className="px-3 py-2">
                    {img ? (
                      ...
                      <div className="size-10 rounded bg-navy/5" />
                    )}
                  </td>
                  <td className="px-3 py-2 font-medium">
                    {title}
                    {r.is_breaking ? (
                      <span className="ml-2 rounded bg-orange px-1.5 py-0.5 text-[10px] font-bold text-white uppercase">
                        Breaking
                      </span>
                    ) : null}
                    {r.is_featured ? (
                      <span className="ml-2 rounded bg-gold px-1.5 py-0.5 text-[10px] font-bold text-navy uppercase">
                        Hero
                      </span>
                    ) : null}
                    {r.is_top10 ? (
                  ...
                  <td className="px-3 py-2 text-xs text-navy/60 hidden md:table-cell">
                    {meta}
                  </td>
    ...
    try {
      ...
      if (resource.table === "articles") {
        ...
        if (selectedCat) {
          ...
          payload.category_slug = selectedCat.slug;
        } else {
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold">
            {isNew ? "Create" : "Edit"} {resource.label.replace(/s$/, "")}
          </h2>
          <button onClick={onClose} className="rounded p-1 hover:bg-navy/5">
            <X className="size-4" />
          </button>
        </div>
        <div className="space-y-4">
            ...
            if (f.type === "textarea") {
              return (
                <div key={f.key}>
                  <label
                    htmlFor={id}
                    className="block text-xs font-semibold uppercase tracking-wider text-navy/60"
                  >
                    {f.label}
                  </label>
                  <textarea
            ...
            if (f.type === "richtext") {
              return (
                <div key={f.key} className="col-span-full">
                  <label className="block text-xs font-semibold uppercase tracking-wider text-navy/60 mb-1">
                    {f.label}
                  </label>
                  <RichTextEditor
                    value={(v as string) ?? ""}
            ...
            if (f.type === "select_sub_district") {
              ...
              return (
                <div key={f.key}>
                  <label
                    htmlFor={id}
                    className={`block text-xs font-semibold uppercase tracking-wider ${hasJila ? "text-navy/60" : "text-navy/30"}`}
                  >
                    {f.label}
                  </label>
                  <select
                    ...
                    onChange={(e) => set(f.key, e.target.value || null)}
                    disabled={!hasJila}
                    className="mt-1 w-full rounded-lg border border-navy/15 px-3 py-2 text-sm disabled:bg-navy/5 disabled:text-navy/40"
                  >
                    <option value="">
                      -- Select Sub-District (Optional) --
                    </option>
                    {subDistricts.map((sd) => (
                  ...
                  {!hasJila && (
                    <p className="text-[10px] text-navy/40 mt-1">
                      Please select a District (Jila) first.
                    </p>
                  )}
                </div>
              );
                ...
                <div key={f.key}>
                  ...
                  <div className="mt-1 flex items-center gap-3">
                    {v ? (
                      ...
                      <div className="size-16 rounded bg-navy/5" />
                    )}
                  ...
                  <input
                    ...
                    className="mt-2 w-full rounded-lg border border-navy/15 px-3 py-2 text-xs"
                  />
                </div>
              );
            }
        ...
        <div className="sticky bottom-0 mt-6 flex gap-2 border-t border-navy/10 bg-white pt-4">
          <button
            onClick={save}
            disabled={busy}
            className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-navy py-2.5 text-sm font-semibold text-paper disabled:opacity-50"
          >
            <Save className="size-4" /> {busy ? "Saving…" : "Save"}
          </button>
          <button
            onClick={onClose}
            className="rounded-lg border border-navy/15 px-4 py-2.5 text-sm"
          >
            Cancel
          </button>
        </div>
